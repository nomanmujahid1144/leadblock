...
interface BlacklistSelectProps {
...
const BLACKLIST_OPTIONS = [
    ...
    { id: 'not-blacklisted', label: 'Not Blacklisted' },
];

const BlacklistSelect: React.FC<BlacklistSelectProps> = ({ selectedBlacklist, onSelect, onReset }) => {
    ...
    return (
        <div className="mb-6" ref={dropdownRef}>
            <div className="flex items-center justify-between mb-3">
                <h4 className="text-sm font-semibold text-text-heading">Blacklisted</h4>
            ...
            <div className="relative">
                ...
                >
                    <span className={selectedBlacklist ? 'text-text-heading' : 'text-neutral-400'}>
                        {selectedBlacklist ? BLACKLIST_OPTIONS.find(o => o.id === selectedBlacklist)?.label : 'Select blacklist status'}
                    </span>
                ...
                {isOpen && (
                    <div className="absolute top-full mt-2 w-full bg-white border border-stroke rounded-lg shadow-lg z-50 animate-fade-in">
                        <div className="px-3 py-2.5 border-b border-neutral-100">
                            <p className="text-sm font-semibold text-text-heading">Select blacklist status</p>
                        </div>
                        <div className="py-2">
                            {BLACKLIST_OPTIONS.map((option) => (
                                <label key={option.id} className="flex items-center gap-3 px-3 py-2 hover:bg-neutral-50 cursor-pointer transition-colors">
                                    <input
                                        type="radio"
                                        name="blacklist"
                                        checked={selectedBlacklist === option.id}
