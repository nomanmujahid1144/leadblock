...
const SENTIMENTS: Sentiment[] = [
    { id: 'other-dmu', label: 'Other DMU' },
    { id: 'neg-not-interested', label: 'NEG not interested' },
    { id: 'neutral', label: 'Neutral' },
    { id: 'pos-interested-followup', label: 'POS interested in follow-up' },
    { id: 'pos-interested-later', label: 'POS interested later' },
    { id: 'pos-maybe-interested', label: 'POS maybe interested' }
];

interface SentimentSelectProps {
    selectedSentiments: string[];
    onToggle: (sentimentId: string) => void;
    onReset: () => void;
}

const SentimentSelect: React.FC<SentimentSelectProps> = ({
    selectedSentiments,
    onToggle,
    onReset
}) => {
    ...
    return (
        <div className="mb-6" ref={dropdownRef}>
            ...
            <div className="flex items-center justify-between mb-3">
                <h4 className="text-sm font-semibold text-text-heading">Sentiment</h4>
                <button
                    onClick={onReset}
            ...
            <div className="relative">
                ...
                >
                    <span className={selectedSentiments.length > 0 ? 'text-text-heading' : 'text-neutral-400'}>
                ...
                {isOpen && (
                    <div className="absolute top-full mt-2 w-full bg-white border border-stroke rounded-lg shadow-lg z-50 animate-fade-in">
                        ...
                        <div className="px-3 py-2.5 border-b border-neutral-100">
                            <p className="text-sm font-semibold text-text-heading">Select a sentiment</p>
                        </div>
                            ...
                            {SENTIMENTS.map((sentiment) => (
                                ...
                                >
                                    <input
                                        ...
                                        onChange={() => onToggle(sentiment.id)}
                                        className="custom-checkbox"
                                    />
                                    <span className="text-sm text-text-heading">{sentiment.label}</span>
                                </label>
                            ))}
                        </div>
