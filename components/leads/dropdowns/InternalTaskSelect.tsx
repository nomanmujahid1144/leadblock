...
interface InternalTaskSelectProps {
    selectedTask: string;
...
const TASK_OPTIONS = [
    { id: 'all', label: 'All' },
    { id: 'no-internal-task', label: 'No Internal Task' },
    { id: 'overdue', label: 'Overdue' },
    { id: 'today', label: 'Today' },
    { id: 'future', label: 'Future' },
];

const InternalTaskSelect: React.FC<InternalTaskSelectProps> = ({ selectedTask, onSelect, onReset }) => {
    ...
    return (
        <div className="mb-6" ref={dropdownRef}>
            <div className="flex items-center justify-between mb-3">
                <h4 className="text-sm font-semibold text-text-heading">Internal Task</h4>
            ...
            <div className="relative">
                ...
                >
                    <span className={selectedTask ? 'text-text-heading' : 'text-neutral-400'}>
                        {selectedTask ? TASK_OPTIONS.find(o => o.id === selectedTask)?.label : 'Select internal task status'}
                    </span>
                ...
                {isOpen && (
                    <div className="absolute top-full mt-2 w-full bg-white border border-stroke rounded-lg shadow-lg z-50 animate-fade-in">
                        <div className="px-3 py-2.5 border-b border-neutral-100">
                            <p className="text-sm font-semibold text-text-heading">Select internal task status</p>
                        </div>
                        <div className="py-2">
                            {TASK_OPTIONS.map((option) => (
                                <label key={option.id} className="flex items-center gap-3 px-3 py-2 hover:bg-neutral-50 cursor-pointer transition-colors">
                                    <input
                                        type="radio"
                                        name="internal-task"
                                        checked={selectedTask === option.id}
...
export default InternalTaskSelect;